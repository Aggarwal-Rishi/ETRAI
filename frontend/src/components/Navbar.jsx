...
import { ShieldCheck, PlusCircle, LayoutDashboard, History, LogOut, Search, Users, CreditCard } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 glass-panel border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <button
            ...
            className="hidden md:flex items-center gap-3 px-3 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800/80 border border-slate-700/60 text-slate-400 hover:text-slate-200 transition-all text-xs w-64 shadow-inner"
          >
            <Search className="w-4 h-4 text-slate-500" />
            <span className="flex-1 text-left">Search reports, claims...</span>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsSearchOpen(true)}
              className="p-2 md:hidden rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 transition-colors"
              title="Search"
            >
