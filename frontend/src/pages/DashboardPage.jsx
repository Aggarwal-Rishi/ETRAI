  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
        ...
        {/* ========================================================================= */}
        <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-[#000D59] via-slate-900 to-slate-950 border border-slate-800 shadow-2xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-[#E88F6B]/20 text-[#E88F6B] border border-[#E88F6B]/30 text-[11px] font-mono font-bold uppercase tracking-wider">
              <Sparkles className="w-3 h-3" /> Live Intelligence Overview
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              {getGreeting()}, {userName}
            </h1>
            <p className="text-xs sm:text-sm text-slate-300">
              {suspiciousWeekCount > 0
                ? `You have ${suspiciousWeekCount} item${suspiciousWeekCount > 1 ? 's' : ''} flagged as suspicious this week requiring editorial review.`
                : 'All recent verifications across your coverage beats are indexed and sealed.'}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 relative z-10 flex-shrink-0">
            <Link
              to="/analysis"
              className="px-5 py-3 bg-[#D97757] hover:bg-[#B0512F] text-white font-bold rounded-xl text-xs shadow-lg shadow-[#D97757]/25 transition flex items-center gap-2 group"
            >
        ...
        {/* ========================================================================= */}
        {/* 2. 4 METRIC CARDS (ALL COMPUTED FROM REAL DB DATA)                        */}
        {/* ========================================================================= */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          
          {/* Card 1: Verified Today */}
          <div className="p-5 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
            <div className="flex items-center justify-between text-slate-400 text-xs font-medium uppercase font-mono">
          ...
          <div className="p-5 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
            <div className="flex items-center justify-between text-slate-400 text-xs font-medium uppercase font-mono">
              ...
              <Film className="w-4 h-4 text-[#E88F6B]" />
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold font-mono text-white">
        ...
        {/* ========================================================================= */}
        <div className="p-6 bg-slate-900/80 border border-slate-800 rounded-3xl space-y-4 shadow-lg">
              ...
              <div className="h-4 w-full bg-slate-950 rounded-full overflow-hidden flex border border-slate-800 p-0.5 gap-0.5">
                {verdictMix.verified.pct > 0 && (
                  <div
                    ...
                    className="h-full bg-[#E4EFE7] rounded-l-full transition-all duration-700"
                  />
                )}
                {verdictMix.suspicious.pct > 0 && (
                  <div
                    ...
                    className="h-full bg-[#F7EEDA] transition-all duration-700"
                  />
                )}
                {verdictMix.false.pct > 0 && (
                  <div
                    ...
                    className="h-full bg-[#F7E3E0] transition-all duration-700"
                  />
                )}
                {verdictMix.insufficient.pct > 0 && (
                  <div
                    ...
                    className="h-full bg-[#E9ECF0] rounded-r-full transition-all duration-700"
                  />
                )}
              </div>

              {/* Legend with Shared VerdictBadge Status Washes */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs pt-1">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#E4EFE7] border border-[#C6DFCF]" />
                  <span className="text-slate-300">Verified Real ({verdictMix.verified.pct}%)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#F7EEDA] border border-[#EBD9AE]" />
                  <span className="text-slate-300">Suspicious ({verdictMix.suspicious.pct}%)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#F7E3E0] border border-[#EBC7C2]" />
                  <span className="text-slate-300">Flagged Fake ({verdictMix.false.pct}%)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#E9ECF0] border border-[#D3D9E1]" />
                  <span className="text-slate-300">Unverified ({verdictMix.insufficient.pct}%)</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="py-6 px-4 text-center bg-slate-950/40 rounded-2xl border border-dashed border-slate-800 text-xs text-slate-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-6 bg-slate-900/80 border border-slate-800 rounded-3xl space-y-4 flex flex-col justify-between">
              ...
              {narrativeClusters.length > 0 ? (
                narrativeClusters.map((cl, idx) => (
                  <div
                    ...
                    className="p-3.5 bg-slate-950/70 border border-slate-800/80 hover:border-indigo-500/40 rounded-2xl cursor-pointer transition space-y-2 group"
                  >
                    <div className="flex items-center justify-between">
                ...
                <div className="py-8 px-4 text-center bg-slate-950/40 rounded-2xl border border-dashed border-slate-800 text-xs text-slate-400 space-y-1">
            ...
            <Link
              to="/fake-news"
              className="text-xs font-semibold text-[#E88F6B] hover:underline flex items-center gap-1 pt-2"
            >
              ...
              {needsReadQueue.length > 0 ? (
                needsReadQueue.map((item) => (
                  <div
                    ...
                    className="p-3.5 bg-slate-950/70 border border-slate-800/80 hover:border-amber-500/40 rounded-2xl cursor-pointer transition flex items-center justify-between gap-3 group"
                  >
        ...
        <div className="p-6 bg-slate-900/80 border border-slate-800 rounded-3xl space-y-4 shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Latest investigations sealed by your newsroom agents.
              </p>
            </div>
            <Link
              to="/history"
            ...
            {recentReports.length > 0 ? (
              recentReports.map((r) => (
                <div
                  ...
                  className="p-4 bg-slate-950/80 hover:bg-slate-850 border border-slate-800/80 hover:border-slate-700 rounded-2xl cursor-pointer transition flex flex-col sm:flex-row sm:items-center justify-between gap-3 group"
                >
                  <div className="space-y-1.5 min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap text-xs text-slate-400">
                      ...
                      <span>·</span>
                      <span className="px-1.5 py-0.2 bg-slate-800 rounded text-[10px] font-mono uppercase">
                        {r.inputType}
                      </span>
                    </div>
              ...
              <div className="py-12 px-4 text-center bg-slate-950/40 rounded-2xl border border-dashed border-slate-800 text-xs text-slate-400 space-y-2">
